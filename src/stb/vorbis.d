...
int stb_vorbis_decode_memory(ubyte *mem, int len, int *channels, short **output);
// decode an entire file and output the data interleaved into a malloc()ed
// buffer stored in *output. The return value is the number of samples
// decoded, or -1 if the file could not be opened or was not an ogg vorbis file.
// When you're done with it, just free() the pointer returned in *output.

stb_vorbis * stb_vorbis_open_memory(ubyte *data, int len,
...
stb_vorbis * stb_vorbis_open_file_section(FILE *f, int close_handle_on_close,
                int *error, stb_vorbis_alloc *alloc_buffer, uint len);
// create an ogg vorbis decoder from an open FILE *, looking for a stream at
